...
const FoldText = ({
  ...
  const segments = useMemo(() => {
    ...
    // Split por carácter, pero agrupando los caracteres de cada palabra en un
    // wrapper nowrap: si no, cada carácter es un inline-block suelto y el
    // navegador puede cortar las palabras a mitad (ej. "LEVANTA / R" sola).
    return text.split("\n").flatMap((line, lineIndex) => {
      ...
      line.split(/( +)/).forEach((word, wordIndex) => {
        if (!word) return;
        if (/^ +$/.test(word)) {
          nodes.push(<span key={`ws-${lineIndex}-${wordIndex}`}> </span>);
          return;
        }
        nodes.push(
          <span
          ...
          >
            {Array.from(word).map((char, charIndex) =>
              renderSegment(char, `segment-${lineIndex}-${wordIndex}-${charIndex}`)
            )}
          </span>
        );
      });
  ...
  const rootStyle: CSSProperties = {
    ...(fontSize !== undefined ? { "--fold-text-font-size": typeof fontSize === "number" ? `${fontSize}px` : fontSize } : {}),
